...
import com.daprlabs.cardstack.R;
...
public class SwipeDeck extends FrameLayout {
    ...
    private int leftViewResourceId;
    private int rightViewResourceId;
    private int topViewResourceId;
    private int bottomViewResourceId;
...
//        Log.i(TAG, "addAndMeasureChild");
        ...
        for (View c : children) {
            ...
            if (leftViewResourceId != 0) child.findViewById(leftViewResourceId).setAlpha(0);
            if (rightViewResourceId != 0) child.findViewById(rightViewResourceId).setAlpha(0);
            if (topViewResourceId != 0) child.findViewById(topViewResourceId).setAlpha(0);
            if (bottomViewResourceId != 0) child.findViewById(bottomViewResourceId).setAlpha(0);
        }
        setZTranslations();
...
//        Log.i(TAG,"positionItem-index:"+index);
//        Log.i(TAG,"positionItem-left:"+left);
//        Log.i(TAG,"positionItem-width:"+width);
//        Log.i(TAG,"positionItem-height:"+height);


        Resources r = getContext().getResources();
        int px = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (getChildCount()-1-index) * CARD_SPACING, r.getDisplayMetrics());
        Log.i(TAG,"positionItem-px:"+px);

        child.layout(left + (px), paddingTop, left + width - (px), paddingTop + height);
...
//                    Log.i(TAG, "onDragProgress - xProgress:"+xProgress+", yProgress:"+yProgress);
            ...
            if (!(rightViewResourceId == 0)) rightView = child.findViewById(rightViewResourceId);
            if (!(leftViewResourceId == 0)) leftView = child.findViewById(leftViewResourceId);
            if (!(topViewResourceId == 0)) topView = child.findViewById(topViewResourceId);
            if (!(bottomViewResourceId == 0)) bottomView = child.findViewById(bottomViewResourceId);
            swipeListener.setLeftView(leftView);
            swipeListener.setRightView(rightView);
            swipeListener.setTopView(topView);
            swipeListener.setBottomView(bottomView);
...
//            addNextCard();
    ...
    public void setLeftViewResourceId(int viewResourceId) {
        leftViewResourceId = viewResourceId;
    }

    public void setRightViewResourceId(int viewResourceId) {
        rightViewResourceId = viewResourceId;
    }

    public void setTopViewResourceId(int viewResourceId) {
        topViewResourceId = viewResourceId;
    }

    public void setBottomViewResourceId(int viewResourceId) {
        bottomViewResourceId = viewResourceId;
    }
